...
impl<'p> Layer<'p> {
    ...
    /// Adds a filled path. `true` on a point marks it as the start of a cubic
    /// Bézier or as its first control point; printpdf reads them in fours.
    fn add_filled_shape<I>(&self, points: I)
    where
        I: IntoIterator<Item = (LayerPosition, bool)>,
    {
        let ring: Vec<_> = points
            .into_iter()
            .map(|(pos, is_control)| (self.transform_position(pos).into(), is_control))
            .collect();
        self.data.layer.add_polygon(printpdf::Polygon {
            rings: vec![ring],
...
impl<'p> Area<'p> {
    ...
    /// Fills a closed path with the given color.
    ///
    /// Positions are relative to the upper left corner of the area. `true` on a
    /// point marks the start of a cubic Bézier or its first control point, so a
    /// curve is four consecutive entries: start, control, control, end.
    pub fn fill_polygon<I>(&self, points: I, color: Color)
    where
        I: IntoIterator<Item = (Position, bool)>,
    {
        self.layer.set_fill_color(Some(color));
        self.layer.add_filled_shape(
            points
                .into_iter()
                .map(|(pos, is_control)| (self.position(pos), is_control)),
        );
    }
